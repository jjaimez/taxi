...
<html lang='es'>
	<head>
		...
		<script type='text/javascript'>function refreshData() { $('#pendiente').prepend($('<div>').load('actualizarEmpresaEjemplo.php?empresa=".$_COOKIE['taxiuser']."'));};
	...
	<body>";
		...
		<div id='pendiente'>";			
			...
			echo "<ul>";
					echo "<li> </li>";
					$query = "SELECT * FROM pedido where empresa='".$_COOKIE['taxiuser']."' and estado = 'pendiente' order by id desc;";
					$rec = mysqli_query($con,$query);
					...
					{ 
						...
						echo "<p><a href='mostrarDireccion.php?latO=".$row->lat_origen."&lngO=".$row->lng_origen."&latD=".$row->lat_destino."&lngD=".$row->lng_destino."' class='lytebox' data-title='Direcciones' data-lyte-options='width:1016 height:816 scrollbars:no'><input type='button' value='Ver mapa'></a>&nbsp;&nbsp;<input type='button' onclick='respuesta(".$row->id.");'value='Responder'></p>";
						echo "</li>";
						mysqli_begin_transaction($con);
	...
	echo "</ul>
        ...
        <div id='camino'>	  
        	<ul>";    
        			echo "<li> </li>";    		
					$query = "SELECT * FROM pedido where empresa='".$_COOKIE['taxiuser']."' and estado = 'en camino' order by id desc;";
					$rec = mysqli_query($con,$query);		
					...
					{ 
						...
						echo "<p><a href='mostrardireccion.php?latO=".$row->lat_origen."&lngO=".$row->lng_origen."&latD=".$row->lat_destino."&lngD=".$row->lng_destino."' class='lytebox' data-title='Direcciones' data-lyte-options='width:1016 height:816 scrollbars:no'><input type='button' value='Ver mapa'></a>&nbsp;&nbsp;<input type='button' onclick='afuera(".$row->id.");'value='Avisar'></p>";
						echo "</li>";						
					}
     ...
     echo "</ul>     	
        ...
        <ul>";
        echo "<li> </li>";
					$query = "SELECT * FROM pedido where empresa='".$_COOKIE['taxiuser']."' and estado = 'afuera' order by id desc;";
					$rec = mysqli_query($con,$query);		
					while($row = mysqli_fetch_object($rec)) // $row = $rec->fetch_object()
					{ 
						echo "<li>";
						...
						echo "<p><a href='mostrardireccion.php?latO=".$row->lat_origen."&lngO=".$row->lng_origen."&latD=".$row->lat_destino."&lngD=".$row->lng_destino."' class='lytebox' data-title='Direcciones' data-lyte-options='width:1016 height:816 scrollbars:no'><input type='button' value='Ver mapa'></a>&nbsp;&nbsp;<input type='button' onclick='viajando(".$row->id.");'value='Finalizar'></p>";
						echo "</li>";
					} 
    echo   "</ul>
    	</div>
        ...
        <ul>";
        echo "<li> </li>";	
					$query = "SELECT * FROM pedido where empresa='".$_COOKIE['taxiuser']."' and estado = 'terminado' order by id desc;";
					$rec = mysqli_query($con,$query);		
					while($row = mysqli_fetch_object($rec)) // $row = $rec->fetch_object()
					{ 
						echo "<li>";
